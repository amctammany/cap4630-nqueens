...
type FitnessFunc = (entity: number[]) => number;
function assignWeights(population: number[][], fitness: FitnessFunc) {
...
export function findNonConflictingPairs(entity: number[]) {
  let score = 0;
...
function mutate(entity: number[]) {
  const n = entity.length;
  return entity.map((q) =>
    Math.random() < MUTATE_PROBABILITY ? getRandomInt(n) : q
  );
}
function createRandomOrganism(n: number) {
  return function () {
    return Array.from({ length: n }).map(() => getRandomInt(n));
  };
}
...
export function genAlgo(
  ...
  let bestFound: number[] | undefined = undefined;
  let bestScore = 0;
...
function reproduce(p1: number[], p2: number[]) {
  const n = p1.length;
